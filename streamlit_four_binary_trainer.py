#!/usr/bin/env python3
"""
Streamlit GUI for Training 4 Binary Classification Models
Interactive Medical X-ray AI Training Interface
"""
...
# Page configuration
st.set_page_config(
    page_title="4 Binary Models Trainer",
    page_icon="🏥",
    layout="wide",
...
class StreamlitBinaryModelsTrainer:
    """
    Streamlit GUI trainer for 4 binary classification models
    """
    ...
    def prepare_data_generators(self, dataset_path: str, batch_size: int):
    ...
    def train_single_model(self, model_key: str, epochs: int, batch_size: int, 
        ...
        try:
            ...
            train_gen, val_gen = self.prepare_data_generators(
                config['dataset_path'], batch_size
            )
...
def main():
    ...
    st.title("🏥 Medical X-ray AI: 4 Binary Models Trainer")
    ...
    # Initialize trainer
    trainer = StreamlitBinaryModelsTrainer()
    ...
    for model_key, config in trainer.model_configs.items():
        ...
        if status['available']:
            checkbox_label = f"✅ {config['name']} ({status['sample_count']} samples)"
