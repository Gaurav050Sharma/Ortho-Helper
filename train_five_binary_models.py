#!/usr/bin/env python3
"""
Five Binary Classification Models Training System
DenseNet121 Architecture with Early Stopping and Dropout
Medical X-ray AI System - Binary Classification Approach (Updated with Bone Fracture Model)
"""
...
class FiveBinaryModelsTrainer:
    """
    Trainer for 5 separate binary classification models:
    1. Normal vs Pneumonia (Chest)
    2. Normal vs Cardiomegaly (Chest)  
    3. Normal vs Arthritis (Knee)
    4. Normal vs Osteoporosis (Knee)
    5. Normal vs Fracture (Bone - Hand/Leg bones only)
    """
    ...
    def __init__(self, base_path='Dataset'):
        ...
        self.model_configs = {
            ...
            'bone_fracture': {
                ...
                'uses_subdirs': True  # This dataset has train/val/test structure
            }
        }
    ...
    def prepare_dataset_generators(self, model_key: str):
        ...
        # Check if this dataset uses subdirectories (train/val/test structure)
        uses_subdirs = config.get('uses_subdirs', False)
        
        if uses_subdirs:
            # Handle datasets with train/val/test structure
            train_path = os.path.join(dataset_path, 'train')
            val_path = os.path.join(dataset_path, 'val')
            test_path = os.path.join(dataset_path, 'test')
            
            if not all(os.path.exists(p) for p in [train_path, val_path, test_path]):
                print(f"❌ Required subdirectories (train/val/test) not found in: {dataset_path}")
                return None, None, None
            ...
            try:
                ...
                print(f"❌ Error loading structured dataset: {str(e)}")
                return None, None, None
        
        else:
            # Handle datasets with simple structure (using validation_split)
    ...
    def train_all_models(self):
        """
        Train all 5 binary classification models
        """
        print("🎯 Starting Training of 5 Binary Classification Models")
        ...
        print(f"\n🎉 Training Complete! {successful_models}/5 models trained successfully.")
    ...
    def update_model_registry(self):
        """
        Update model registry with all 5 binary models
        """
    ...
    def save_training_summary(self):
        ...
        summary_path = f"models/five_binary_training_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def plot_training_results(self):
        ...
        try:
            ...
            for idx, (model_key, model_data) in enumerate(self.models.items()):
                ...
                # Plot accuracy
                axes[0, idx].plot(history.history['accuracy'], label='Training Accuracy', color='blue')
                axes[0, idx].plot(history.history['val_accuracy'], label='Validation Accuracy', color='red')
                axes[0, idx].set_title(f"{model_data['config']['name']} - Accuracy")
                ...
                # Plot loss
                axes[1, idx].plot(history.history['loss'], label='Training Loss', color='blue')
                axes[1, idx].plot(history.history['val_loss'], label='Validation Loss', color='red')
                axes[1, idx].set_title(f"{model_data['config']['name']} - Loss")
            ...
            plt.tight_layout()
            plot_path = f"models/five_binary_training_plots_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def main():
    ...
    """
    print("🏥 Medical X-ray AI - 5 Binary Models Training System")
    print("DenseNet121 with Early Stopping and Dropout")
    print("=" * 80)
    
    # Initialize trainer
    trainer = FiveBinaryModelsTrainer()
    
    # Train all models
    successful_count = trainer.train_all_models()
    
    if successful_count > 0:
        print(f"\n🎊 SUCCESS! {successful_count}/5 models trained and saved.")
        ...
        print("\n✅ All models are ready for comprehensive medical X-ray diagnosis!")
